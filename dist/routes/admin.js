...
router.get('/users', async (_req, res) => {
    const result = await db_1.pool.query(`SELECT id, email, role, full_name, company, reset_requested, created_at
     FROM users
...
router.post('/users', async (req, res) => {
    const { full_name, email, company, password, role } = req.body;
    ...
    const result = await db_1.pool.query(`INSERT INTO users (email, password_hash, role, full_name, company)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, email, role, full_name, company, created_at`, [normalizedEmail, passwordHash, role, full_name.trim(), company ?? null]);
